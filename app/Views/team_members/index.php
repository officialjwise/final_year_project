    ...
    <div class="card">
        <div class="page-title clearfix">
            ...
            <div class="title-button-group">
                ...
                <?php
                $can_manage_users = $login_user->is_admin || get_array_value($login_user->permissions, "can_add_or_invite_new_team_members");
                
                // Check if institution user can manage users
                if ($login_user->user_type === "client" && $login_user->client_id) {
                    $can_manage_users = $can_manage_users || ($login_user->is_primary_contact == 1) || 
                                      (strpos($login_user->client_permissions, 'institution_user_management') !== false);
                }
                
                if ($can_manage_users) {
                    // Only show import and invitation for super admin and team members
                    ...
                    // Show add member button for all users who can manage users
                    $button_text = ($login_user->user_type === "client") ? app_lang('add_institution_member') : app_lang('add_team_member');
                    echo modal_anchor(get_uri("team_members/modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . $button_text, array("class" => "btn btn-default", "title" => $button_text));
                }
                ?>
            </div>
        </div>
