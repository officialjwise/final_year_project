...
class Activity_logs_model extends Model {
    ...
    protected function _get_clean_value($options_or_value, $key = "") {
        ...
        if (is_string($value)) {
            ...
            if (
                preg_match('/(?<!\w)\b(OR|TABLE|UNION(?:\s+ALL)?|INSERT|DELETE|UPDATE|EXEC|DROP|ALTER|TRUNCATE|REPLACE|LOAD_FILE|OUTFILE|INTO|GROUP\s+BY|ORDER\s+BY|HAVING|CASE|LIKE|--|#|\/\*)\b(?!\w)/i', $value)
                || preg_match('/[";#()]/', $value)  // Dangerous characters (excluding commas)
                || preg_match('/0x[0-9a-f]+/i', $value)  // Hexadecimal pattern
                || preg_match('/\/\*.*\*\//', $value)  // SQL comments
                || preg_match('/;.*;/', $value)  // Multiple semicolons
                || preg_match('/\b\d+\s*[!=<>]\s*\d+\b(?!,)/', $value)  // Detect numeric comparisons (e.g., 1=1)
                || preg_match('/%[0-9a-f]{2}/i', $value)  // URL encoding like %27 for '
                || preg_match('/(?<!\w)-\d+\s+(OR|DELETE|UPDATE|INSERT|DROP|ALTER|UNION)\b/i', $value)  // Detect negative numbers followed by SQL keywords
            ) {
                $backtrace = $this->get_backtrace();
