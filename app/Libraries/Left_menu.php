...
class Left_menu {
    ...
    private function _get_sidebar_menu_items($type = "") {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            // Allow team members and institution users with management permissions
            $can_view_team_members = get_array_value($this->ci->login_user->permissions, "hide_team_members_list") != "1";
            
            // Check if institution user can manage users
            if ($this->ci->login_user->user_type === "client" && $this->ci->login_user->client_id) {
                $can_view_team_members = $can_view_team_members || 
                                       ($this->ci->login_user->is_primary_contact == 1) || 
                                       (strpos($this->ci->login_user->client_permissions, 'institution_user_management') !== false);
            }
            
            if ($can_view_team_members) {
