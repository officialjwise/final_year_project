...
class Clients extends Security_Controller {
    ...
    // Method for institution admins to add new users to their institution
    function add_institution_user_modal() {
        // Only allow client users (institution users)
        if ($this->login_user->user_type !== "client") {
            app_redirect("forbidden");
        }
        ...
        $view_data['client_info'] = $this->Clients_model->get_one($client_id);
        $view_data['is_institution_admin'] = true;
        $view_data["available_menus"] = get_available_menus_for_clients_dropdown();
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("client_contacts", 0, false, "client")->getResult();
        
        return $this->template->view('clients/contacts/add_user_modal', $view_data);
    }

    // Save new institution user
    function save_institution_user() {
        // Only allow client users (institution users)
        if ($this->login_user->user_type !== "client") {
            app_redirect("forbidden");
        }
        ...
        $password = $this->request->getPost('login_password');
        if ($password) {
            $user_data["password"] = password_hash(clean_data($password), PASSWORD_DEFAULT);
        } else {
            $user_data["password"] = "";
        }
        ...
        if ($can_access_everything == 1) {
            ...
            $user_data['client_permissions'] = $specific_permissions ? $specific_permissions : get_setting("default_permissions_for_non_primary_contact");
        }
