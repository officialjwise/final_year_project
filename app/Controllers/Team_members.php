...
class Team_members extends Security_Controller {
    ...
    function __construct() {
        parent::__construct();
        // Allow both team members and institution users - detailed permission checks in individual methods
    ...
    public function index() {
        if (!($this->can_view_team_members_list() || $this->can_manage_institution_users())) {
            app_redirect("forbidden");
        }

        $view_data["show_contact_info"] = $this->can_view_team_members_contact_info();

        // Use appropriate custom fields based on user type
        if ($this->can_manage_institution_users() && !$this->login_user->is_admin) {
    ...
    private function access_only_admin_or_member_creator() {
        if (!($this->login_user->is_admin || get_array_value($this->login_user->permissions, "can_add_or_invite_new_team_members") || $this->can_manage_institution_users())) {
            app_redirect("forbidden");
        }
    }

    // Check if current user can manage users for their institution
    private function can_manage_institution_users() {
        // Allow if user is a client (institution) with primary contact status or admin privileges within the institution
        if ($this->login_user->user_type === "client" && $this->login_user->client_id) {
            $user_info = $this->Users_model->get_one($this->login_user->id);
            // Allow if user is primary contact or has institution_user_management permission
            return $user_info->is_primary_contact == 1 || 
                   (strpos($user_info->client_permissions, 'institution_user_management') !== false);
        }
        return false;
    }

    // Check if current user can manage a specific user within their institution
    private function can_manage_this_institution_user($user_id) {
        if (!$this->can_manage_institution_users()) {
            return false;
        }
        
        if ($user_id) {
            $target_user = $this->Users_model->get_one($user_id);
            // Allow management only if target user belongs to same institution
            return $target_user && $target_user->client_id == $this->login_user->client_id;
        }
        
        return true; // For new user creation
    }
    ...
    function modal_form() {
        ...
        if ($id && !$this->can_manage_this_institution_user($id)) {
            app_redirect("forbidden");
        }

        // Determine if creating for institution
        $view_data['is_institution_user'] = $this->can_manage_institution_users() && !$this->login_user->is_admin;
        
        // For super admins, provide option to create institution users
        $view_data['is_super_admin'] = $this->login_user->is_admin;
        
        // Get institutions dropdown for super admin
        if ($this->login_user->is_admin) {
            $view_data['institutions_dropdown'] = $this->_get_institutions_dropdown();
        }
        
        // Get appropriate custom fields based on user type
        if ($view_data['is_institution_user']) {
            $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("client_contacts", 0, $this->login_user->is_admin, $this->login_user->user_type)->getResult();
        } else {
    ...
    function add_team_member() {
        ...
        if ($this->login_user->is_admin && $this->request->getPost('user_type') === 'client') {
            // For institution users created by super admin
            $validation_rules["client_id"] = "required";
        } else if (!$this->can_manage_institution_users() || $this->login_user->is_admin) {
            // For staff users (either created by super admin or team members)
            $validation_rules["job_title"] = "required";
        ...
        // Determine user type based on who is creating the user
        if ($this->can_manage_institution_users() && !$this->login_user->is_admin) {
            // Institution user creating a new member - create as client contact
            $user_data["user_type"] = "client";
            $user_data["client_id"] = $this->login_user->client_id;
            $user_data["client_permissions"] = $this->request->getPost('client_permissions') ?: 'limited_access';
            $user_data["is_primary_contact"] = 0; // New users can't be primary contact by default
        } else if ($this->login_user->is_admin && $this->request->getPost('user_type') === 'client') {
            // Super admin creating institution user
            $user_data["user_type"] = "client";
            $user_data["client_id"] = $this->request->getPost('client_id');
            $user_data["client_permissions"] = $this->request->getPost('client_permissions') ?: 'limited_access';
            $user_data["is_primary_contact"] = 0;
        } else {
            // Super admin or team member creating - create as staff
        ...
        if ($this->login_user->is_admin && $role === "admin" && $user_data["user_type"] === "staff") {
        ...
        //add a new team member or institution user
        $user_id = $this->Users_model->ci_save($user_data);
        if ($user_id) {
            //user added, now add the job info for staff users
            ...
            // Save custom fields based on user type
            $custom_field_context = $user_data["user_type"] === "staff" ? "team_members" : "client_contacts";
            save_custom_fields($custom_field_context, $user_id, $this->login_user->is_admin, $this->login_user->user_type);
    ...
    function list_data() {
        ...
        // Determine user type and custom fields based on who is accessing
        if ($this->can_manage_institution_users() && !$this->login_user->is_admin) {
            // Institution user - show institution contacts
            $user_type = "client";
            ...
            $options = array(
                ...
                "user_type" => $user_type,
                "client_id" => $this->login_user->client_id, // Only show users from same institution
            ...
            // Admin or team member - show staff and optionally institution users for admin
            $user_type = $this->request->getPost("user_type") ?: "staff";
            
            if ($user_type === "client" && $this->login_user->is_admin) {
                // Super admin viewing institution users
                $custom_fields = $this->Custom_fields_model->get_available_fields_for_table("client_contacts", $this->login_user->is_admin, $this->login_user->user_type);
                $options = array(
                    "status" => $this->request->getPost("status"),
                    "user_type" => $user_type,
    ...
    function _row_data($id) {
        ...
        if (!$this->can_manage_this_institution_user($id) && !$this->can_view_team_members_list()) {
            app_redirect("forbidden");
        }
        
        // Determine custom fields based on user type
        $user_data = $this->Users_model->get_one($id);
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            //or if institution user, check if they can manage this user
            if (!$this->can_view_team_members_list() && $this->login_user->id != $id && !$this->can_manage_this_institution_user($id)) {
                app_redirect("forbidden");
            }

            //we have an id. view the team_member's profie
            // Allow both staff and client users to be viewed if accessible
            $user_data = $this->Users_model->get_one($id);
            $user_type = $user_data ? $user_data->user_type : "staff";
            
            $options = array("id" => $id, "user_type" => $user_type);
            ...
            if ($user_info) {
                ...
                $can_update_team_members_info = $this->can_update_team_members_info($id) || $this->can_manage_this_institution_user($id);

                $view_data['show_general_info'] = $can_update_team_members_info;
                $view_data['show_job_info'] = false;

                // Job info only for staff users
                if ($user_type === "staff" && ($this->login_user->is_admin || $user_info->id === $this->login_user->id || $this->has_job_info_manage_permission())) {
    ...
    private function _get_institutions_dropdown() {
        $institutions = $this->Clients_model->get_details(array("client_groups" => "", "status" => "active"))->getResult();
        $institutions_dropdown = array("" => "- " . app_lang('select_institution') . " -");
        
        foreach ($institutions as $institution) {
            $institutions_dropdown[$institution->id] = $institution->company_name;
        }
        
        return $institutions_dropdown;
    }
}
